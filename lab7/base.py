...
class Coalition(object):
    ...
    def __get_product(self, prod_type: str) -> Product or None:
        for prod in self.products:
            if prod.type == prod_type:
                return prod
        return None
    ...
    def __build_share(self, share: Dict[str, Dict[str, float]]) -> Dict[str, Any]:
        ...
        for prod_type in product_types:
            prod = self.__get_product(prod_type)
            ...
            if prod_share:
                if prod_share[Coalition.PROD_VALUE] < prod.value:
                    ...
                    raise ValueError("Expected agent value for product type %s cannot exceed product value %4.2f"
    ...
    def coalition_value(self) -> float:
        ...
        for md in self.member_data:
            ...
            for prod in self.products:
                prod_contributions[prod.type] += member_share[prod.type][Coalition.PROD_CONTRIB]

        coalition_val = 0
        for prod in self.products:
            prod_contrib = prod_contributions[prod.type]
            if prod_contrib >= prod.price:
                coalition_val += prod.value
            else:
                return 0
