...
while n < num_episodes:
	...
	obs = env.reset()
	old_state = str(obs[:3])
	old_goal_state = str(obs[3:])

	action_id = np.argmax(policy_function(old_state))  # Exploit learned values
	...
	while not done:
		...
		action_id = np.argmax(policy_function(old_state))  # Exploit learned values
		new_action = all_possible_actions[action_id]

		if q_table.get(old_state) is None:
			q_table[old_state] = np.zeros(env.numactions)

		if q_table.get(current_state) is None:
			q_table[current_state] = np.zeros(env.numactions)

		old_value = q_table[old_state][get_action_id(action)]
		...
		q_table[old_state][get_action_id(action)] = new_value
